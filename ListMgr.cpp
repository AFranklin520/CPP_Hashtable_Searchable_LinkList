...
//bool ListMgr::Find(Person* p)
//{
//	string n = p->GetName();
//	PersonNode* np = first;
//	if (count == 0) return;
//	while (!(np->GetNext()->GetPerson()->GetName() < n))
//	{
//		//Checks if name and birthdays match before returning Person*
//		if (np->GetPerson()->GetName() == n && np->GetPerson()->GetBirthDay().GetYearMonthDay() == p->GetBirthDay().GetYearMonthDay())
//		{
//			return true;
//			break;
//		}
//		np = np->GetNext(); //else goes to next person in node
//	}
//	return false;
//}
